...
## ContractManager 合同生成（2026-05-18）
- **模板路径**：`public/contract_template.docx`
- **模板商品占位符**：`{{商品明细}}`（第1行数据格），由 JS 端渲染 Tab 分隔的多行文本
- **docxtemplater 分隔符**：必须显式指定 `delimiters: { start: '{{', end: '}}' }`（v3.68.7 默认分隔符从 `{{`/`}}` 变更为 `{`/`}`）
- **invoices 字段**：用 `product_info`（商品名称）、`quantity`（数量）、`unit_price`（单价）、`amount`（金额）四项
- **明细渲染格式**：`商品名称(全角补齐16字符)\t公斤\t数量\t¥单价\t¥金额`，每行一笔交易
- **生成流程**：`handleGenerateContract` 内部直接查询 invoices 明细，不依赖 `tradePreview` 缓存
